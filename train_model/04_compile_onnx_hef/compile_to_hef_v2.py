...
def write_model_script():
    ...
    """
    # normalization([mean]*3, [std]*3) bakes (x-mean)/std into the HEF.
    # Training: transforms.Normalize([0.5,0.5,0.5],[0.5,0.5,0.5]) after ToTensor()
    # = (x/255 - 0.5)/0.5 = (x - 127.5)/127.5 on uint8 [0,255] input.
    # With this baked in, HEF accepts raw uint8 RGB and maps to [-1,1] on-chip.
...
def main():
    ...
    if args.calib_npy and os.path.exists(args.calib_npy):
        ...
        # Calibration must match inference input range.
        # With normalization baked in, inference sends uint8 [0,255].
        # Use float [0,255] here; replace with real LP crops for production.
        calib = rng.uniform(0.0, 255.0,
    ...
    calib_nhwc = (calib_nhwc / 127.5) - 1.0   # [0,255] → [-1,1] to match training   # (N,3,75,300) → (N,75,300,3)
